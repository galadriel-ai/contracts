...
def check_metrics(metrics: dict):
    ...
    assert metrics["transactions_sent"] == 13, "transactions_sent should be 13"
    assert metrics["chats_count"] == 8, "chats_count should be 8"
    assert metrics["chats_read"] == 8, "chats_read should be 8"
    assert metrics["chats_answered"] == 8, "chats_answered should be 8"
    assert (
    ...
    assert metrics["chats_marked_as_done"] == 0, "chats_marked_as_done should be 0"
    ...
    assert (
        metrics["knowledgebase_index_marked_as_done"] == 0
    ), "knowledgebase_index_marked_as_done should be 0"
    assert (
    ...
    assert (
        metrics["knowledgebase_query_marked_as_done"] == 0
    ), "knowledgebase_query_marked_as_done should be 0"
